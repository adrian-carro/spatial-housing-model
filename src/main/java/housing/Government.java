...
/**************************************************************************************************
 * Class to represent the government. This is the class where taxation policy is encoded
 *
...
public class Government {
    ...
    public Government(Config config) { this.config = config; }
	...
	/**
	 * Calculates the income tax due in one year for a given gross annual income. Doesn't account for married couple's
     * allowance.
	 * 
	...
	/**
	 * Calculate the class 1 National Insurance Contributions due on a given annual income (under PAYE).
	 * 
	 * @param grossIncome Gross annual income in pounds
	 * @return Annual class 1 NICs due.
	 */
	double class1NICsDue(double grossIncome) {
		return bandedPercentage(grossIncome,
                                data.Government.nationalInsurance.bands,
                                data.Government.nationalInsurance.rates);
	}
     ...
     * each band. So, for example, if x lies somewhere between x1 and x2, f(x) would be p1x1 + p2(x-x1)
	 * 
